...
def sum_complexity(data, k=2):
    result = 0
    for line in data:
        num = int(line[:3])
        complexity = num * find_shortest_sequence(line, proxies=k)
        result += complexity
...
if __name__ == '__main__':
    ...
    res = sum_complexity(test_1)
    print(res)
    assert res == 126384

    res = sum_complexity(inp)
    print(res)

    res = sum_complexity(inp, k=25)
    print(res)
